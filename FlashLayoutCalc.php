...
if (! isset($BadBlocksMarginMin)) $BadBlocksMarginMin=0;
...
if ((isset($BadBlocksMarginPercent)) && ($BadBlocksMarginPercent>0))
{
printf ("BB handling per partition:\n");
printf ("\treserved ............: %d%%\n", $BadBlocksMarginPercent);
printf ("\treserved min ........: %d ebl\n", $BadBlocksMarginMin);
}
...
foreach ($arFlashLayout as $partname => $partdesc)
{
	...
	$part_size_blocks = $part_size/$FlashEraseBlockSize;

	printf ("\toffset: 0x%08X %10d   (%7s) (%5s ebl)\n", $offset,		$offset,	get_human_value($offset),    ($offset/$FlashEraseBlockSize));
	printf ("\tsize:   0x%08X %10d   (%7s) (%5s ebl)\n", $part_size,	$part_size,	get_human_value($part_size), $part_size_blocks);
	if ((isset($BadBlocksMarginPercent)) && ($BadBlocksMarginPercent>0))
	{
	    $reserved_part_size_blocks  = ceil($part_size_blocks * $BadBlocksMarginPercent / 100);
	    if ($reserved_part_size_blocks < $BadBlocksMarginMin)
	    {	
		$reserved_part_size_blocks = $BadBlocksMarginMin;
	    }
	    $usable_part_size = ($part_size_blocks - $reserved_part_size_blocks) * $FlashEraseBlockSize;
	printf ("\tusable: 0x%08X %10d   (%7s) (%5s ebl)\n", $usable_part_size,	$usable_part_size,	get_human_value($usable_part_size), ($usable_part_size/$FlashEraseBlockSize));
	printf ("\t        (%d ebl = %s reserved for BB)\n", $reserved_part_size_blocks, get_human_value($reserved_part_size_blocks*$FlashEraseBlockSize));
	}
	echo "----------------------------------------------------------------------\n";
